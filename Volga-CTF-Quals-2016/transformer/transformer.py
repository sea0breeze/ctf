...
blockmessage = 'LengthOfBlock-16'
mes = ''.join(reversed(blockmessage))
def f1(m, k):
    res = ''
    for i in range(16):
        res += chr(ord(m[i]) ^ ord(xor1[k][i]))
    return res

def f2(m):
    res = ''
    for i in range(16):
        res += set602268[ord(m[i])]
    return res

def f3(m):
    res = ''
    for i in range(16):
        res += m[15-ord(shuffle[i])]
    return res

def f4(m):
    res = ''
    for i in range(0,16,4):
        res += complexencrypt(m[i:i+4])
    return res

def complexencrypt(m):
    a = ord(m[0])
    b = ord(m[1])
    c = ord(m[2])
    d = ord(m[3])
    return chr(ord(set602394[a])^b^c^ord(set602494[d]))+chr(ord(set602494[a])^ord(set602394[b])^c^d)+chr(a^ord(set602494[b])^ord(set602394[c])^d)+chr(a^b^ord(set602494[c])^ord(set602394[d]))
...
mes = f3(mes)
mes = f1(mes, k)
print ''.join(reversed(mes)).encode('hex')
